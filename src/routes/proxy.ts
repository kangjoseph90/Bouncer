...
proxyRoutes.use('/v1/*', async (c, next) => {
  ...
  if (!tokenMatched) {
    return c.json({ error: { message: 'Bouncer API 키가 누락되었거나 형식이 잘못되었습니다.', type: 'invalid_request_error' } }, 401);
  }

  const user = getUserByApiKey(tokenMatched[1]);
  if (!user || user.status !== 'active') {
    return c.json({ error: { message: '만료/정지되었거나 유효하지 않은 Bouncer API 키입니다.', type: 'invalid_api_key' } }, 401);
  }
...
proxyRoutes.post('/v1/chat/completions', async (c) => {
  ...
  if (user.credit_balance <= 0) {
    return c.json({ error: { message: '본인의 할당량(크레딧)을 모두 소진하셨습니다.', type: 'insufficient_quota' } }, 429);
  }

  // 2. 글로벌 쿼터 예외 처리
  if (!checkGlobalQuotaAllowed(0)) {
    return c.json({ error: { message: '서버 전체의 제공 할당량(GLOBAL QUOTA)이 모두 소진되어 일시적으로 이용할 수 없습니다.', type: 'insufficient_quota' } }, 429);
  }
  ...
  if (!modelId) {
    return c.json({ error: { message: '요청 본문에 "model" 필드가 누락되었습니다.', type: 'invalid_request_error' } }, 400);
  }
  ...
  if (!modelConfig) {
    return c.json({ error: { message: `지원하지 않는 모델명(Alias)입니다: ${modelId}`, type: 'invalid_request_error' } }, 400);
  }

  if (!process.env[modelConfig.targetKeyEnv]) {
    return c.json({ error: { message: `해당 연결된 다중 모델 혹은 단일 모델('${modelConfig.targetModel || modelId}')은 서버에 API 키(${modelConfig.targetKeyEnv})가 설정되지 않아 현재 비활성화되어 있습니다.`, type: 'server_configuration_error' } }, 503);
  }

  const limitCheck = checkRateLimits(user.arca_id, modelId);
  if (!limitCheck.allowed) {
    return c.json({ error: { message: limitCheck.error, type: 'rate_limit_exceeded' } }, 429);
  }
  ...
  const handlerInstance = getHandler(modelConfig.handler, {
    c, user, modelConfig, body
  });

  if (!handlerInstance) {
    // 핸들러가 없으면 락 해제 후 에러 반환 (임시 락 해제 모듈 필요하나 여기서는 단순화)
    return c.json({ error: { message: `Handler '${modelConfig.handler}' not found for model ${modelId}`, type: 'server_error' } }, 501);
  }
