...
authRoutes.get("/token", (c) => {
  ...
  const data = generateVerificationToken(ip);
...
authRoutes.post("/verify", async (c) => {
  try {
    ...
    if (!isValidToken(token)) {
      return c.json(
        { success: false, error: "만료되었거나 유효하지 않은 토큰입니다." },
        400,
      );
    }
    ...
    if (!checkTokenRateLimit(token)) {
      return c.json(
        {
          success: false,
          error: `이 토큰으로 너무 많은 인증 시도를 했습니다 (최대 ${config.AUTH_TOKEN_VERIFY_LIMIT}회). 새 토큰을 발급받아주세요.`,
        },
        429,
      );
    }
      ...
      return c.json(
        {
          success: false,
          error:
            "서버에 ARCA_POST_URL(인증 게시글 위치)이 아직 설정되지 않았습니다. 관리자에게 문의하세요.",
        },
        500,
      );
    }
    ...
    try {
      ...
      return c.json(
        { success: false, error: e.message || "게시글 크롤링 중 오류" },
        500,
      );
    }
      ...
      return c.json(
        {
          success: false,
          error:
            "지정된 게시글에서 해당 토큰을 포함한 유효한 프로필(유동닉 제외)을 찾지 못했습니다." +
    ...
    if (existing) {
        ...
        return c.json(
          {
            success: false,
            error:
              "관리자에 의해 영구 정지된 계정입니다. API를 사용할 수 없습니다.",
          },
          403,
        );
      }
      // 기존 유저면 새 키 발급 (기존 키 파기 및 상태 active 복구)
      const data = revokeAndReissue(
        profileData.arcaId,
      ...
      if (counts.total >= config.GLOBAL_MAX_USERS) {
        return c.json(
          { success: false, error: "유저 등록 한도 초과 (GLOBAL_MAX_USERS)" },
          403,
        );
      }
      if (counts.active >= config.GLOBAL_MAX_ACTIVE_USERS) {
        return c.json(
          {
            success: false,
            error: "활성 유저 한도 초과 (GLOBAL_MAX_ACTIVE_USERS)",
          },
          403,
        );
      }
    ...
    return c.json(
      { success: false, error: "인증 처리 중 서버 오류가 발생했습니다." },
      500,
    );
  }
});
