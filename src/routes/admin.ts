...
import { getSetting, setSetting, adminSearchUser, adminSuspendUser } from '../db/queries';
...
adminRoutes.use('/*', async (c, next) => {
  ...
  if (!config.ADMIN_PASSWORD) {
    return c.json({ success: false, error: '서버 환경변수에 ADMIN_PASSWORD가 설정되지 않아 관리자 기능을 사용할 수 없습니다.' }, 403);
  }

  if (!tokenMatched || tokenMatched[1] !== config.ADMIN_PASSWORD) {
    return c.json({ success: false, error: '관리자 비밀번호가 일치하지 않습니다.' }, 401);
  }
...
adminRoutes.post('/settings/arca-post-url', async (c) => {
  try {
    ...
    if (typeof url !== 'string') {
      return c.json({ success: false, error: '올바른 url 값이 제공되지 않았습니다.' }, 400);
    }
    setSetting('ARCA_POST_URL', url);
    return c.json({ success: true, message: '게시글 URL이 성공적으로 업데이트되었습니다.' });
  } catch (e) {
    return c.json({ success: false, error: '잘못된 요청 형식이거나 데이터 베이스 오류입니다.' }, 400);
  }
});
...
adminRoutes.post('/models/reload', (c) => {
  try {
    loadModels();
    return c.json({ success: true, message: 'models.json 설정을 성공적으로 다시 불러왔습니다.' });
  } catch(e) {
...
adminRoutes.post('/users/:arcaId/suspend', (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: `${arcaId} 유저가 영구 정지 처리되었으며, 기존 발급된 API 키는 모두 즉시 무력화되었습니다.` });
  } catch(e) {
    return c.json({ success: false, error: '유저 차단 처리 중 에러가 발생했습니다.' }, 500);
  }
});
