...
dashboardRoutes.get("/dashboard", (c) => {
  ...
  if (!apiKey || !apiKey.startsWith("bnc-")) {
    return c.json(
      { success: false, error: "유효한 API 키가 제공되지 않았습니다." },
      401,
    );
  }
  ...
  if (!user) {
    return c.json(
      { success: false, error: "존재하지 않거나 정지된 계정입니다." },
      401,
    );
  }
...
dashboardRoutes.post("/revoke", (c) => {
  ...
  if (!user) {
    return c.json(
      { success: false, error: "존재하지 않거나 이미 정지/파기된 계정입니다." },
      401,
    );
  }
  ...
  try {
    ...
    return c.json({
      success: true,
      message:
        "API 키가 성공적으로 파기되었습니다. 더 이상 이 키로 프록시를 사용할 수 없습니다.",
    });
  } catch (error) {
    console.error("Revoke error:", error);
    return c.json(
      { success: false, error: "파기 처리 중 서버 오류가 발생했습니다." },
      500,
    );
  }
});
