...
export function checkRateLimits(
  ...
  if (!modelConfig)
    return { allowed: false, error: "존재하지 않거나 비활성화된 모델입니다." };

  // Global Check
  if (globalState.activeRequests >= config.GLOBAL_MAX_CONCURRENCY)
  ...
  // User Check
  if (uState.activeRequests >= config.USER_MAX_CONCURRENCY)
...
export function recordRequestStart(arcaId: string, modelId: string) {
  ...
  globalState.modelRequests.set(
    modelId,
    (globalState.modelRequests.get(modelId) || 0) + 1,
  );
