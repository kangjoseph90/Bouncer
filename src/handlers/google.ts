...
export class GoogleHandler extends BaseHandler {
  async handleAction(): Promise<Response> {
    ...
    if (bodyArgs.temperature !== undefined) geminiPayload.generationConfig.temperature = bodyArgs.temperature;
    if (bodyArgs.top_p !== undefined) geminiPayload.generationConfig.topP = bodyArgs.top_p;
    if (bodyArgs.max_tokens !== undefined || bodyArgs.max_completion_tokens !== undefined) {
      geminiPayload.generationConfig.maxOutputTokens = bodyArgs.max_tokens || bodyArgs.max_completion_tokens;
    }
    if (bodyArgs.stop) {
      geminiPayload.generationConfig.stopSequences = Array.isArray(bodyArgs.stop) ? bodyArgs.stop : [bodyArgs.stop];
    }
    ...
    const baseUrl = this.targetUrl.endsWith('/') ? this.targetUrl.slice(0, -1) : this.targetUrl;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return new Response(JSON.stringify(errorData), { status: response.status, headers: response.headers });
      }
      ...
      this.releaseLock();
      return this.c.json({ error: { message: `Google Gemini 업스트림 통신 오류: ${e.message}`, type: 'server_error' } }, 502);
    }
  }
}
